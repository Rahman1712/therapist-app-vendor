...
//	================================================
	...
	public List<TherapistUserDto> getTherapistByIds(List<Long> ids) {
		return therapistRepository.findByIdIn(ids).stream().map(TherapistToUsersUtils::therapistToTherapistUserDto).toList();
	}
	
	public List<TherapistInfoUserDto> getTherapistInfoUserByIds(List<Long> ids) {
		return therapistRepository.findByIdIn(ids).stream().map(TherapistUtils::therapistToTherapistInfoUserDto).toList();
	}
	
//	public List<TherapistUserDto> getTherapistUsers() {
//		return therapistRepository.findAll()
//				.stream()
//				.sorted(Comparator.comparingInt(therapist -> therapist.getAppointments().size()))
...
//	}
	public List<TherapistUserDto> getTherapistUsers() {
	    return therapistRepository.findAll()
	            .stream()
	            .sorted(Comparator.comparingInt(therapist -> 
	            	((Therapist) therapist).getAppointments().size())
	                    .reversed()) // Sort from high to low
	            .map(TherapistToUsersUtils::therapistToTherapistUserDto)
	            .toList();
	}
	...
	public List<TherapistUserDto> getTherapistsByCategoryName(String categoryName) {
		List<Therapist> therapists = therapistRepository.findByTherapistInfo_Categories_Name(categoryName);
		return therapistListToDtoList(
				 therapists.stream()
                .sorted(Comparator.comparingInt(therapist -> therapist.getAppointments().size()))
                .collect(Collectors.toList())
				);
...
//    			.filter(Therapist::isActivated)
	...
	@Transactional
	public TherapistDto updateProfileData(Long therapistId, UserUpdateRequest request) {
		var therapist = therapistRepository.findById(therapistId)
				.orElseThrow(() -> new TherapistException("Therapist not found with id "+therapistId));
		
		therapist.setFullname(request.getFullname());
		therapist.setMobile(request.getMobile());
		therapist.setPassword(passwordEncoder.encode(request.getPassword()));
		
		return Optional.of(therapistRepository.save(therapist)).map(TherapistUtils::therapistToTherapistDtoMini).orElse(null);
	}
